...
> **"20년(2026~2046) 동안 어떤 경제 위기(스태그플레이션, 대공황, 신용경색)에서도 계좌를 깨지 않고 연복리 18%를 창출하는 궁극의 100% 순수 무레버리지 퀀트 자산배분 시스템"**

---

## 🌟 1. 프로젝트 개요 (Executive Summary)

본 프로젝트는 글로벌 6대 자산배분 유니버스(정적 올웨더, 켈러 동적모멘텀, 안토나치 듀얼모멘텀, 매크로 4분면, 추세추종 등) **총 32개 전략을 23.3년(2003~2026, 5,880 거래일)** 동안 왕복 30bp 실전 수수료 및 1-Day Lag 환경에서 전수 백테스트하고 검증한 마스터 퀀트 시스템입니다.

### 🏆 최종 마스터 20년 연금 포트폴리오 성과

$$\mathbf{\text{Master Pure Monthly (55/30/15)}} = \mathbf{55\%}\text{ IC(V1)} + \mathbf{30\%}\text{ DualMom(V0)} + \mathbf{15\%}\text{ BAA-G4(V2)}$$

| 핵심 지표 | S&P 500 (SPY) | 전통 60/40 포트폴리오 | **Master Pure Monthly (최종 마스터)** ⭐ |
|---|:---:|:---:|:---:|
| **CAGR (연복리수익률)** | 11.91% | 8.98% | **18.13%** (시장 대비 +6.2%p) |
| **연간 변동성** | 18.51% | 10.69% | **17.05%** |
| **Sharpe 지수 (무위험 2% 차감)** | 0.701 | 0.858 | **1.063** |
| **MDD (최대낙폭)** | -55.19% | -31.39% | **-21.83% (SPY의 1/3 이하)** |
| **Calmar 지수 (CAGR / \|MDD\|)** | 0.216 | 0.286 | **0.831** |
| **연간 턴오버** | 0.0x | 0.0x | **2.9x (초저비용)** |
| **2008 글로벌 금융위기 성과** | -36.79% | -16.73% | **-0.67% (원금 보존)** |
| **2022 금리/인플레 폭등기 성과** | -18.18% | -16.39% | **+15.66% (압도적 플러스 수익!)** |
| **3년 롤링 원금 손실 확률** | 11.7% | 4.6% | **0.0% (적자 제로)** |
| **운용 방식** | 매수 후 보유 | 정적 보유 | **100% 완전 월 1회 (10분 컷)** |
...
│   ├── strategies/           # 32개 정적/동적/매크로 전략 구현체
│   ├── production/           # 실시간 미국 ETF 주문표 생성기
...
- **100% 무레버리지**: 전 전략 1.0배수 정수 배분 및 가중치 합 1.0(100%) 보장.
- **전수 단위 테스트 통과 (6/6 OK)**:
  ```bash
